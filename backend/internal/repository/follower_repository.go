...
// GetFollowerCounts returns the number of followers and following for a user
func (r *FollowerRepository) GetFollowerCounts(userID string) (int, int, error) {
	var followersCount, followingCount int

	// Get followers count
	err := r.DB.QueryRow("SELECT COUNT(*) FROM followers WHERE followed_id = ? AND status = 'accepted'", userID).Scan(&followersCount)
	if err != nil {
		return 0, 0, fmt.Errorf("failed to get followers count: %w", err)
	}

	// Get following count
	err = r.DB.QueryRow("SELECT COUNT(*) FROM followers WHERE follower_id = ? AND status = 'accepted'", userID).Scan(&followingCount)
	if err != nil {
...
func (r *FollowerRepository) AcceptFollowRequest(followedID, followerID string) error {
...
func (r *FollowerRepository) RejectFollowRequest(followedID, followerID string) error {
